...
def load_artifacts():
    ...
    artifacts["metadata"] = load_json(METADATA_PATH)
    return artifacts


def ensure_dataframe_from_input(data: Any, feature_names: List[str] = None) -> pd.DataFrame:
    ...
    # Try coercing object columns to numeric where appropriate
    for col in df.columns:
        if df[col].dtype == object:
            try:
                df[col] = pd.to_numeric(df[col], errors='raise')
            except Exception:
                # leave as-is; model may error later which we propagate
                pass
...
def startup_load():
    ...
    try:
        ...
        # If metadata doesn't include feature_names, optionally set them manually here
        ...
        raise RuntimeError(f"Failed to load artifacts at startup: {e}")
...
def health():
    okay = True
    details = {"pipeline_loaded": PIPELINE is not None, "estimator_only": ESTIMATOR_ONLY is not None}
    return {"status": "ok" if okay else "error", "details": details}


@app.post("/predict", response_model=List[PredictResponse])
def predict(payload: Union[Dict[str, Any], List[Dict[str, Any]]] = Body(..., examples={
    "single": {
    ...
    """
    Accepts either a single JSON object with the selected features, or a list of such objects.
    Returns the same structure as your previous `predict_from_pipeline` output.
    """
